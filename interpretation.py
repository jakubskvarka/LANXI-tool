...
class Parser():
    def __init__(self,name_of_file,channel_info):
        ...
        for index, (key, value) in enumerate(data.items()):
            ...
            print(f"sample period is{sample_period}")
        ...
        with h5py.File(hdf5_filename, 'w') as hdf5_file:
            ...
            for index, (key, value) in enumerate(data.items()):
                ...
                scaled_samples = (samples * scale_factor) / 2 ** 23
                # Uloží vzorky
                print(f"Sample nr {index+1} name is {self.active_channels[index]['name']}")
                samples_dataset = signal_group.create_dataset(f'{self.active_channels[index]["name"]}', data=scaled_samples)
                # Uloží metadáta pre každý signál
                # Signal ID
                samples_dataset.attrs['signal_id'] = key
                # Jednotka
                samples_dataset.attrs['InterpretationUnit'] = self.active_channels[index]["unit"]
                # Vzorkovacia perióda
                ...
                #scale_factor
                samples_dataset.attrs['InterpretationScaleFactor'] = scale_factor
